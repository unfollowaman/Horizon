  ...
  return (
    <header className="absolute top-0 left-0 right-0 z-50 flex justify-center w-full">
      ...
      <div className="hidden md:flex w-full max-w-7xl justify-between items-center px-8 py-6 mx-auto relative z-20">
        {/* Logo */}
        <Link to="/" className="text-2xl tracking-tight font-normal text-white focus:outline-none focus:ring-2 focus:ring-white rounded-sm" style={{ fontFamily: "'Instrument Serif', serif" }}>
          Horizon
        </Link>
        ...
        <nav className="flex items-center gap-6">
          {navLinks.filter(link => link.showOnDesktop).map((link, index) => (
            <Link key={index} to={link.path} className="text-sm text-[#9A9AA8] hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-white rounded-sm">
              {link.label}
            </Link>
          ))}
        </nav>
        ...
        {/* Get Started */}
        <Link to="/" className="liquid-glass rounded-full px-6 py-2.5 text-sm text-white hover:scale-[1.03] transition-transform focus:outline-none focus:ring-2 focus:ring-white">
          Get Started
        </Link>
      </div>
