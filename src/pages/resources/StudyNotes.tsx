...
const StudyNotes: React.FC = () => {
  ...
  useEffect(() => {
    const fetchResources = async () => {
      ...
      // Perform a join with the chapters table to get chapter details
      ...
      if (error) {
        ...
        const mappedResources: Resource[] = data.map((item: any) => {
          ...
          // Format title with chapter info if available
          let title = item.title;
          if (item.chapters) {
             const chapterNum = item.chapters.chapter_number;
             const chapterName = item.chapters.chapter_name;

             if (chapterNum && chapterName) {
                 title = `Chapter ${chapterNum}: ${chapterName}`;
             } else if (chapterName) {
                 title = chapterName;
             }
          }
    ...
    <div className="w-[min(96vw,1600px)] mx-auto px-[clamp(16px,2vw,32px)] max-md:pt-[10px] md:-mt-[20px] pb-[clamp(24px,3vw,48px)]">
      ...
      {loading ? (
        ...
        <div className="neu-recessed rounded-2xl p-8 text-center">
          ...
          <p className="text-caption">Try selecting a different class or subject.</p>
        </div>
      ) : (
