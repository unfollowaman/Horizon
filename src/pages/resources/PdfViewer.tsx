...
pdfjs.GlobalWorkerOptions.workerSrc = new URL(
  ...
  import.meta.url,
).toString();import React, { useState, useEffect, useRef, useCallback } from 'react';
import { useParams, useNavigate, Link } from 'react-router-dom';
import { Document, Page, pdfjs } from 'react-pdf';
import { TransformWrapper, TransformComponent } from 'react-zoom-pan-pinch';
import 'react-pdf/dist/Page/AnnotationLayer.css';
...
const PdfViewer: React.FC = () => {
  ...
  // We need a ref to access setTransform from outside the render prop
  ...
  const handleTransformed = useCallback((ref: { state: { positionX: number, positionY: number, scale: number }, instance?: { contentComponent?: HTMLElement } }) => {
    if (!ref.state) return;
    ...
    // The easiest way is to calculate maxScrollY:
    // If we have ref.instance.contentComponent, we can get its height.
    const contentHeight = ref.instance?.contentComponent?.offsetHeight || container.scrollHeight;
    const scaledContentHeight = contentHeight * scale;
    const maxScrollY = Math.max(0, scaledContentHeight - clientHeight);
    
    if (maxScrollY > 0 && sliderContainerRef.current) {
      // positionY goes from 0 down to -maxScrollY
      const currentScrollY = Math.abs(positionY);
      const progress = Math.min(1, Math.max(0, currentScrollY / maxScrollY));
  ...
  const handleSliderDrag = useCallback((clientY: number) => {
    ...
    // Now update the TransformWrapper state
    // We need to know maxScrollY
        ...
        <div className={styles.transformWrapperContainer}>
          ...
          >
            {({ zoomIn, zoomOut, setTransform }) => {
              // Store setTransform in ref to use in drag handlers
